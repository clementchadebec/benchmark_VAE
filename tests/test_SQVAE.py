...
from pythae.models import SQVAE, SQVAEConfig, AutoModel
...
@pytest.fixture(params=[SQVAEConfig(), SQVAEConfig(latent_dim=4)])
...
class Test_Model_Building:
    ...
    def test_build_model(self, model_configs):
        ...
        with pytest.raises(AssertionError):
            SQVAEConfig(temperature_decay=10)
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = SQVAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = SQVAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = SQVAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = SQVAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = SQVAE(model_configs_no_input_dim, decoder=custom_decoder)

    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):

        model = SQVAE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
        ...
        model = SQVAE(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = SQVAE(model_configs, decoder=custom_decoder)
...
class Test_SQVAETraining:
    ...
    def test_checkpoint_saving_during_training(
        ...
        training_dir = os.path.join(
            dir_path, f"SQVAE_training_{trainer._training_signature}"
        )
    ...
    def test_vae_training_pipeline(self, tmpdir, vae, train_dataset, training_configs):
        ...
        training_dir = os.path.join(
            dir_path, f"SQVAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
...
class Test_SQVAE_Generation:
    ...
    @pytest.fixture()
    def ae_model(self):
        return SQVAE(SQVAEConfig(input_dim=(1, 28, 28), latent_dim=4))
