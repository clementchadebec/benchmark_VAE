...
class Test_Model_Saving:
    def test_default_model_saving(self, tmpdir, model_configs):
        ...
        # reload model
        model_rec = SVAE.load_from_folder(dir_path)
...
class Test_NLL_Compute:
    ...
    def test_nll_compute(self, svae, demo_data, nll_params):
        nll = svae.get_nll(data=demo_data['data'], n_samples=nll_params[0], batch_size=nll_params[1])
...
class Test_SVAE_Training:
    ...
    def test_final_model_saving(
        ...
        # check reload full model
        model_rec = SVAE.load_from_folder(os.path.join(final_dir))
