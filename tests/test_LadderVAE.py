...
from tests.data.custom_architectures import (
    Decoder_LadderVAE_MLP_Custom,
    Encoder_LadderVAE_MLP_Custom,
    NetBadInheritance,
)
...
def custom_encoder(model_configs):
    return Encoder_LadderVAE_MLP_Custom(model_configs)


@pytest.fixture
def custom_decoder(model_configs):
    return Decoder_LadderVAE_MLP_Custom(model_configs)
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_configs, bad_net):
        with pytest.raises(BadInheritanceError):
            model = LadderVAE(model_configs, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = LadderVAE(model_configs, decoder=bad_net)

    def test_raises_no_input_dim(
        self, model_configs_no_input_dim, custom_encoder, custom_decoder
    ):
        with pytest.raises(AttributeError):
            model = LadderVAE(model_configs_no_input_dim)

        with pytest.raises(AttributeError):
            model = LadderVAE(model_configs_no_input_dim, encoder=custom_encoder)

        with pytest.raises(AttributeError):
            model = LadderVAE(model_configs_no_input_dim, decoder=custom_decoder)
    ...
    def test_build_custom_arch(self, model_configs, custom_encoder, custom_decoder):
        ...
        model = LadderVAE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
        ...
        model = LadderVAE(model_configs, encoder=custom_encoder)

        assert model.encoder == custom_encoder
        assert not model.model_config.uses_default_encoder
        assert model.model_config.uses_default_decoder

        model = LadderVAE(model_configs, decoder=custom_decoder)
...
class Test_Model_Saving:
    def test_default_model_saving(self, tmpdir, model_configs):
        ...
        # reload model
        model_rec = LadderVAE.load_from_folder(dir_path)
...
class Test_Model_forward:
    ...
    @pytest.fixture
    def lvae(self, model_configs, demo_data):
        model_configs.input_dim = tuple(demo_data["data"][0].shape)
        return LadderVAE(model_configs)

    def test_model_train_output(self, lvae, demo_data):
...
class Test_LadderVAE_Training:
    ...
    def lvae(self, model_configs, custom_encoder, custom_decoder, request):
        # randomized
    ...
    def optimizers(self, request, lvae, training_configs):
        if request.param is not None:
            optimizer = request.param(
                lvae.parameters(), lr=training_configs.learning_rate
            )
    ...
    def test_lvae_train_step(
        self, lvae, train_dataset, training_configs, optimizers
    ):
        trainer = BaseTrainer(
            model=lvae,
    ...
    def test_lvae_main_train_loop(
        self, tmpdir, lvae, train_dataset, training_configs, optimizers
    ):
    ...
    def test_checkpoint_saving(
        ...
        if not lvae.model_config.uses_default_decoder:
        ...
        if not lvae.model_config.uses_default_encoder:
        ...
        model_rec = lvae.load_from_folder(os.path.join(checkpoint_dir))
    ...
    def test_checkpoint_saving_during_training(
        ...
        training_dir = os.path.join(
            dir_path, f"LadderVAE_training_{trainer._training_signature}"
        )
    ...
    def test_final_model_saving(
        ...
        # check reload full model
        model_rec = LadderVAE.load_from_folder(os.path.join(final_dir))
    ...
    def test_lvae_training_pipeline(
        self, tmpdir, lvae, train_dataset, training_configs
    ):
        ...
        pipeline = TrainingPipeline(
            model=lvae, training_config=training_configs
        )
        ...
        training_dir = os.path.join(
            dir_path, f"LadderVAE_training_{pipeline.trainer._training_signature}"
        )
        assert os.path.isdir(training_dir)
