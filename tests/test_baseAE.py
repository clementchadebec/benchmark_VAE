...
@pytest.fixture(
    params=[
        BaseAEConfig(input_dim=784, latent_dim=10),
        BaseAEConfig(input_dim=100, latent_dim=5),
        BaseAEConfig(input_dim=1e4, latent_dim=5),
    ]
)
...
class Test_Model_Building:
    ...
    def test_raises_bad_inheritance(self, model_config_with_input_dim, bad_net):
        with pytest.raises(BadInheritanceError):
            model = BaseAE(model_config_with_input_dim, encoder=bad_net)

        with pytest.raises(BadInheritanceError):
            model = BaseAE(model_config_with_input_dim, decoder=bad_net)
    ...
    def test_raises_no_input_dim(
        ...
        with pytest.raises(AttributeError):
            model = BaseAE(model_configs_no_input_dim, encoder=custom_encoder)
    ...
    def test_build_custom_arch(
        ...
        model = BaseAE(model_config_with_input_dim, encoder=custom_encoder)
