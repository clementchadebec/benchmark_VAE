...
class Test_Model_Saving:
    def test_default_model_saving(self, tmpdir, model_configs):
        ...
        # reload model
        model_rec = Adversarial_AE.load_from_folder(dir_path)
...
class Test_Adversarial_AE_Training:
    ...
    def test_checkpoint_saving(
        ...
        encoder_decoder_optimizer = deepcopy(trainer.encoder_decoder_optimizer)
        ...
        assert set(["model.pt", "encoder_decoder_optimizer.pt", "discriminator_optimizer.pt", "training_config.json"]).issubset(
            set(files_list)
        )
        ...
        model_rec = Adversarial_AE.load_from_folder(os.path.join(checkpoint_dir))
        ...
        encoder_decoder_optim_rec_state_dict = torch.load(os.path.join(checkpoint_dir, "encoder_decoder_optimizer.pt"))
        ...
        assert all(
            [
                ...
                for (dict_rec, dict_optimizer) in zip(
                    encoder_decoder_optim_rec_state_dict["param_groups"],
                    encoder_decoder_optimizer.state_dict()["param_groups"],
                )
            ]
        )
        ...
        assert all(
            [
                dict_rec == dict_optimizer
                for (dict_rec, dict_optimizer) in zip(
                    encoder_decoder_optim_rec_state_dict["state"], encoder_decoder_optimizer.state_dict()["state"]
                )
            ]
        )
    ...
    def test_final_model_saving(
        ...
        # check reload full model
        model_rec = Adversarial_AE.load_from_folder(os.path.join(final_dir))
