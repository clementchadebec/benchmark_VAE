...
class Encoder(BaseEncoder):
    def __init__(self, args: dict):
        ...
        self.layers = nn.Sequential(
                nn.Conv2d(1, 16, 4, 2, padding=1),
                nn.Softplus(),
                nn.Conv2d(16, 32, 4, 2, padding=1), nn.Softplus(),
                nn.Conv2d(32, 32, 4, 2, padding=2), nn.Softplus(),
            )
        ...
        self.embedding = nn.Linear(450, self.latent_dim)
        self.log_var = nn.Linear(450, self.latent_dim)
...
class Decoder(BaseDecoder):
    def __init__(self, args: dict):
        ...
        self.layers = nn.Sequential(
            nn.ConvTranspose2d(32, 32, 3, 2, padding=1),
            nn.Softplus(),
    ...
    def forward(self, z: torch.Tensor, output_layer_levels: List[int] = None):
        ...
        out = self.layers(out.reshape(-1, 32, 4, 4))
...
def main(args):
    ...
    try:
        if args.dataset == "mnist":
            ...
            logger.info(f"\nLoading {args.dataset} data...\n")
            train_data = np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_train.amat")).reshape(-1, 1, 28, 28)
            eval_data = np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_valid.amat")).reshape(-1, 1, 28, 28)
            test_data = np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_test.amat")).reshape(-1, 1, 28, 28)
