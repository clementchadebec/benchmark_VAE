...
def main(args):
    ...
    try:
        if args.dataset == "mnist":
            ...
            logger.info(f"\nLoading {args.dataset} data...\n")
            train_data = torch.tensor(np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_train.amat"))).type(torch.float)
            eval_data = torch.tensor(np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_valid.amat"))).type(torch.float)
            test_data = torch.tensor(np.loadtxt(os.path.join(PATH, f"data/{args.dataset}", "binarized_mnist_test.amat"))).type(torch.float)
