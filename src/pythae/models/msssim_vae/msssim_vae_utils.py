...
class MSSSIM(torch.nn.Module):
    ...
    def _gaussian(self, sigma):
        gauss = torch.Tensor(
            [np.exp(-(x - self.window_size//2)**2/float(2*sigma**2)) for x in range(self.window_size)])
    ...
    def forward(self, img1, img2):
        ...
        if img1.shape[-1] < 4:
            weights = torch.FloatTensor([1.]).to(img1.device)
            ...
            weights = torch.FloatTensor([0.3117, 0.3384, 0.2675, 0.0824]).to(img1.device)
