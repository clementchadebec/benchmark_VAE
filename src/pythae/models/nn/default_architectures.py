...
class Encoder_GMVAE_MLP(BaseEncoder):
    def __init__(self, args: dict):
        BaseEncoder.__init__(self)
        self.input_dim = args.input_dim
        self.latent_dim = args.latent_dim
        ...
        self.embedding_z = nn.Linear(512, self.gmm_latent_dim)
        self.log_var_z = nn.Linear(512, self.gmm_latent_dim)

        self.embedding_w = nn.Linear(512, self.latent_dim)
        self.log_var_w = nn.Linear(512, self.latent_dim)
        ...
        for i in range(max_depth):
            ...
            if output_layer_levels is not None:
                if i+1 in output_layer_levels:
                    output[f'embedding_layer_{i+1}'] = out
            if i+1 == self.depth:
                output['embedding_z'] = self.embedding_z(out)
                output['log_covariance_z'] = self.log_var_z(out)

                output['embedding_w'] = self.embedding_w(out)
                output['log_covariance_w'] = self.log_var_w(out)
