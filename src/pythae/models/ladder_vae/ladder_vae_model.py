...
class LadderVAE(VAE):
    ...
    Args:
        model_config(LadderVConfig): The IWAE configuration seting the main 
    ...
    def __init__(
        ...
        self.ladder_levels = model_config.ladder_levels
        self.ladder_latent_dims = model_config.ladder_latent_dims

        assert all(encoder.depth >=  self.ladder_levels > 0), (
                f'Cannot output layer deeper than depth ({self.depth}) or with non-positive '\
                    f'indice. Got ({output_layer_levels}). Consider amending the encoder or change '
                    "the 'ladder_levels' argument in the model configuration."
                )

        assert len(self.ladder_latent_dims) == len(self.ladder_levels), (
            f"The number of latent dim in the ladder ({len(self.ladder_latent_dims)}) must equal "
            f"the number of levels in the ladder ({len(self.ladder_levels)}). Amend model "
            "configuration."
        )
