...
class AdversarialTrainer(BaseTrainer):
    ...
    Args:
        ...
        encoder_decoder_optimizer (~torch.optim.Optimizer): An instance of `torch.optim.Optimizer` 
    ...
    def __init__(
        ...
        if encoder_decoder_optimizer is None:
            encoder_decoder_optimizer = self.set_default_encoder_decoder_optimizer(model)

        else:
            encoder_decoder_optimizer = self._set_optimizer_on_device(
    ...
    def set_default_encoder_decoder_optimizer(self, model: BaseAE) -> torch.optim.Optimizer:

        optimizer = torch.optim.Adam(
            itertools.chain(model.encoder.parameters(), model.decoder.parameters()),
            lr=self.training_config.learning_rate,
            weight_decay=self.training_config.encoder_decoder_optim_decay
            )
    ...
    def save_checkpoint(self, dir_path, epoch: int):
        ...
        # save optimizers
        torch.save(
            deepcopy(self.encoder_decoder_optimizer.state_dict()),
            os.path.join(checkpoint_dir, "encoder_decoder_optimizer.pt"),
        )
        torch.save(
